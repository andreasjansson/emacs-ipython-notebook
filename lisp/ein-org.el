...
(defun* ein2:org-goto-link (notebook created
                                    &key
                                    worksheet-index
                                    search
                                    &allow-other-keys)
  (if created
      (ein2:log 'info "Linked notebook did not exist.  Created a new one.")
    (if worksheet-index
        (ein2:notebook-worksheet-open-ith notebook worksheet-index
                                         #'pop-to-buffer)
      (pop-to-buffer (ein2:notebook-buffer notebook)))
    (when search
      (goto-char (point-min))
...
`org-add-link-type'."
  (let ((link (read link-path)))
    (destructuring-bind (&key url-or-port name &allow-other-keys)
        link
      (ein2:notebooklist-open-notebook-by-name name url-or-port
                                              #'ein2:org-goto-link link))))

;;;###autoload
(defun ein2:org-store-link ()
...
node `(org) External links' and Info node `(org) Search options'"
  (ein2:and-let* (((ein2:worksheet-buffer-p))
                 (notebook (ein2:get-notebook))
                 (name (ein2:notebook-name notebook))
                 (link (list :url-or-port (ein2:get-url-or-port)
                             :name name))
                 (description name))
    (ein2:aif (ein2:notebook-worksheet-index notebook)
        (unless (= it 0)
...
(eval-after-load "org"
  '(progn
     (org-add-link-type "ipynb" 'ein2:org-open)
     (add-hook 'org-store-link-functions 'ein2:org-store-link)))
...
;; When `ein2:org-open' or `ein2:org-store-link' is called for opening
