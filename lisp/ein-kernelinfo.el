...
(defclass ein2:kernelinfo ()
  ((kernel
    :initarg :kernel :type ein2:$kernel
...
(defun ein2:kernelinfo-new (kernel get-buffers)
  "Make a new `ein2:kernelinfo' instance based on KERNEL and GET-BUFFERS."
  (let ((kerinfo (make-instance 'ein2:kernelinfo)))
    (oset kerinfo :kernel kernel)
    (oset kerinfo :get-buffers get-buffers)
    (ein2:kernelinfo-setup-hooks kerinfo)
    kerinfo))

(defun ein2:kernelinfo-setup-hooks (kerinfo)
  "Add `ein2:kernelinfo-update-*' to `ein2:$kernel-after-*-hook'."
  (with-slots (kernel) kerinfo
    (push (cons #'ein2:kernelinfo-update-all kerinfo)
          (ein2:$kernel-after-start-hook kernel))
    (push (cons #'ein2:kernelinfo-update-ccwd kerinfo)
          (ein2:$kernel-after-execute-hook kernel))))

(defun ein2:kernelinfo-update-all (kerinfo)
  "Update KERINFO slots by triggering all update functions."
  (ein2:log 'debug "EIN2:KERNELINFO-UPDATE-ALL")
  (ein2:log 'debug "(ein2:kernel-live-p kernel) = %S"
           (ein2:kernel-live-p (oref kerinfo :kernel)))
  (ein2:kernelinfo-update-ccwd kerinfo)
  (ein2:kernelinfo-update-hostname kerinfo))

(defun ein2:kernelinfo-update-ccwd (kerinfo)
  ...
  (ein2:kernel-request-stream
   ...
   (lambda (cwd kerinfo)
     (with-slots (kernel get-buffers) kerinfo
       (setq cwd (ein2:kernel-filename-from-python kernel cwd))
       ...
       (when (file-accessible-directory-p cwd)
         (mapc (lambda (buffer)
               ...
               (ein2:filter #'buffer-live-p
                           (ein2:funcall-packed get-buffers))))))
   (list kerinfo)))

(defun ein2:kernelinfo-update-hostname (kerinfo)
